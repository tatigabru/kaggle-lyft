...
from progs.configs import BEV_SHAPE, DATA_ROOT, IMG_SIZES, NUM_CLASSES, OUTPUT_ROOT
from progs.transforms import augment_and_show
...
class BevCocoDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, fold: int, df: pd.DataFrame, 
        ...
        super(BevCocoDataset, self).__init__()  # inherit it from torch Dataset
        self.fold = fold
        self.df = df
...
def get_masks_from_boxes(sample_token, bev_shape, voxel_size, boxes, classes, z_offset=-2):
    """Helper, gets COCO-style masks and labels from the SDK lyft boxes"""
...
def get_boxes_from_masks(masks):
    """ Helper, gets orthogonal boxes from masks """
    coco_boxes = []
    for mask in masks:
        pos = np.nonzero(mask)
        xmin = np.min(pos[1])
...
def plot_image_mask(image, image_aug, mask_rgb, mask_aug_rgb):
...
def test_dataset_augs(train_df, data_folder, level5data):
    ...
    # dataset     
    train_dataset = BevCocoDataset(fold=0, df=train_df, 
    ...
    im, target = train_dataset[10]    
    target = remove_tiny_instances(target, threshold=1)
    ...
    transformations = [A.ShiftScaleRotate(shift_limit=0.0625, scale_limit=0.2, rotate_limit=15, 
                       interpolation=1, border_mode=cv2.BORDER_CONSTANT, value=0, mask_value=0, p=0.5),
                       ...
                       A.Transpose(p=0.2),
                       # crop and resize  
                       A.RandomSizedCrop((img_size-50, img_size+50), img_size, img_size, w2h_ratio=1.0, 
                                        interpolation=1, always_apply=False, p=0.2),                       
                       A.Resize(img_size, img_size, interpolation=1, p=1),      
                       #A.Normalize(),
                       ]

    check_trans = [A.ShiftScaleRotate(shift_limit=0.0625, scale_limit=0.1, rotate_limit=10, 
                       interpolation=cv2.INTER_LANCZOS4, border_mode=cv2.BORDER_CONSTANT, value=0, mask_value=0, p=1),
                    A.RandomSizedCrop((img_size-50, img_size), img_size, img_size, w2h_ratio=1.0, interpolation=cv2.INTER_LANCZOS4, always_apply=False, p=0.5),
                    A.HorizontalFlip(p=0.5),
                    A.VerticalFlip(p=0.5),
                    A.RandomRotate90(p=0.5),
                    A.Transpose(p=0.5), 
                    A.Resize(img_size, img_size, interpolation=cv2.INTER_LANCZOS4, p=1),                      
                    ] 

    bbox_params={'format':'pascal_voc', 'min_area': 5, 'min_visibility': 0.5, 'label_fields': ['category_id']} 
    augs = A.Compose(transformations, bbox_params=bbox_params, p=1)       
    ...
    aug_img = augmented['image']    
    one_aug_mask = augmented['mask']

    # devide one augmented mask
    aug_masks = (one_aug_mask[:, :, None] -1 == np.arange(len(boxes))[None, None, :]).astype(np.uint8)
    print(aug_masks.shape)

    # glue aug masks together
    #one_aug_mask = np.zeros_like(aug_masks[0])
    #for i, augmask in enumerate(aug_masks):
    #    one_aug_mask += (augmask > 0).astype(np.uint8) * (i+1)   
    aug_boxes = get_boxes_from_masks(masks)
    for box in boxes:
        visualize_bbox(im, box)
    for augbox in aug_boxes:
        visualize_bbox(aug_img, augbox)    
    
    mask_rgb = label2rgb(one_mask, bg_label=0)            
    mask_aug_rgb = label2rgb(one_aug_mask, bg_label=0)                   
    plot_image_mask(im, augmented['image'], mask_rgb, mask_aug_rgb)
    
    # test individual masks
    for num in range(len(boxes)):
        mask_rgb = label2rgb(masks[num], bg_label=0)            
        mask_aug_rgb = label2rgb(aug_masks[num], bg_label=0)  
        plot_image_mask(im, augmented['image'], mask_rgb, mask_aug_rgb)
    

    for num in range(5):
        augment_and_show(augs, im, masks=masks, bboxes=boxes, categories=labels, 
                        category_id_to_name=['instances'], show_title=False)    
...
def main():
    ...
    train_dataset = BevCocoDataset(fold=0, df=train_df, 
                                    level5data = level5data,
                                    debug=True, img_size=bev_shape[0], 
                                    input_dir=data_folder, 
                                    transforms = None,
                                    bev_shape = bev_shape,
                                    voxel_size = voxel_size, 
    ...
    test_dataset_augs(train_df, data_folder, level5data)
