...
def get_smp_model(encoder='resnet50', num_classes = NUM_CLASSES+1):
...
def validate(model, model_name: str, dataloader_valid, class_weights, epoch: int, 
    ...
    with torch.no_grad():
        ...
        for iter_num, (img, target, sample_ids) in enumerate(progress_bar):
            ...
            if iter_num == 0:
                visualize_predictions(img, prediction, target, validations_dir, model_name, epoch)                   
    print("Epoch {}, Valid Loss: {}".format(epoch, np.mean(val_losses) ))
        
    return np.mean(val_losses)   


def validate_metric(model, model_name: str, dataloader_valid, class_weights, epoch: int, 
             validations_dir: str, save_oof=True):
    """
    Validate model at the epoch end 
    ...
    with torch.no_grad():
        ...
        for iter_num, (img, target, sample_ids) in enumerate(progress_bar):
            ...
            preds = np.round(preds*255).astype(np.uint8)

            print(f'preds shape: {preds.shape}, gts shape: {gts.shape}')
            ious, iou_mean = iou_numpy(preds, gts, print_table=False)
            ious.append(ious)
            iou.append(iou_mean) 
            ...
            if iter_num == 0:
                visualize_predictions(img, prediction, target, validations_dir, model_name, epoch)                   
    print("Epoch {}, mIoU: {}".format(epoch, np.mean(val_losses), np.mean(iou) ))
        
    return np.mean(ious, axis = 1), np.mean(iou)  
...
def main():
    ...
    #model = fpn_resnet50(num_classes=len(classes)+1) # one more class for background, 0 on the masks
    ...
    train(model, model_name='unet_resnet50_512', data_folder=data_folder, 
          fold=2, debug=False, img_size=IMG_SIZE,
          epochs=30, batch_size=10, num_workers=4, resume_weights='', resume_epoch=0)
